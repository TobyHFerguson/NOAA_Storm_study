...
hurt <- by_event %>% summarize(injuries = sum(INJURIES), fatalities=sum(FATALITIES)) %>% gather("type", "count", injuries, fatalities) %>% 
  group_by(type) %>% 
  top_n(5, count)
ggplot(data=hurt, mapping=aes(x=EVTYPE, y=count)) + geom_col() +labs(title = "Most Harmful Events in USA", x="Event Type") + geom_text(aes(x=EVTYPE, y=count, label=count), hjust=-0.3) + ylim(0, 25000)+facet_grid(type ~ .)+coord_flip()
```
...
damage <- by_event %>%
  ...
  group_by(type) %>% 
  top_n(5, value)
ggplot(data=damage, mapping=aes(x=EVTYPE, y=value)) + geom_col()+labs(title = "Storm Related Damage in USA")+labs(y="Cost in $") + geom_text(aes(x=EVTYPE, y=value, label=value), hjust=-0.2) +facet_grid(type ~ ., labeller = label_both)+ylim(0,2e+11)+coord_flip()
```
