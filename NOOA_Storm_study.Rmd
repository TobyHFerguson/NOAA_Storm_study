...
hurt <- by_event %>% summarize(injuries = sum(INJURIES), fatalities=sum(FATALITIES)) %>% gather("type", "count", injuries, fatalities) %>% top_n(10, count)
ggplot(data=hurt, mapping=aes(x=EVTYPE, y=count, fill=type)) + geom_col() +labs(title = "Most Harmful Events in USA")
```
...
ggplot(data=damage, mapping=aes(x=YEAR, y=value, fill=type)) + geom_col()+labs(title = "Storm Related Damage in USA")+labs(y="Cost in $")
```
